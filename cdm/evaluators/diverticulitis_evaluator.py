...
class DiverticulitisEvaluator(PathologyEvaluator): 
    def __init__(self, grounded_treatment: List[str], grounded_diagnosis: str, hadm_id: int):
        ...
        self.neutral_lab_tests = [lab for lab in self.neutral_lab_tests if lab not in self.required_lab_tests['Inflammation']]
    ...
    def score_imaging(self, region: str, modality: str):
        if region == "abdomen":
            ...
            if modality == "ultrasound" or modality == "us" or modality == "mri":
                ...
                return True
            self.explanations["Imaging"] = "INCORRECT MODALITY: None of US/MRI/CT imaging were ordered"
        else: 
    ...
    def score_treatment(self):
        ...
        if alt_procedure_checker(ALTERNATE_DRAINAGE_KEYWORDS_DIVERTICULITIS, self.answers["Treatment"]):
        ...
        if procedure_checker(COLECTOMY_PROCEDURES_KEYWORDS, self.answers["Treatment"]) or \
            alt_procedure_checker(ALTERNATE_COLECTOMY_KEYWORDS, self.answers["Treatment"]):
